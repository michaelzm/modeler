<template>
  <div class="process-activity flex-row">
    ...
    <transition name="fade">
      <div class="input-informations flex-column" v-if="this.isActivityClicked">
        <div class="input-data-container flex-row center-hor" @click="markActive('input-data')">
          ...
          <div
            class="connecting-to-center-line-hor"
            v-bind:class="{inputDataMarkLine: isInputDataSelected}"
          ></div>
        </div>
        <div class="input-it-container flex-row center-hor" @click="markActive('input-it')">
          ...
          <div
            class="connecting-to-center-line-hor"
            v-bind:class="{inputItMarkLine: isInputItSelected}"
          ></div>
        </div>
      </div>
    </transition>
    ...
    <transition name="fade">
      <div
        class="output-informations noselect flex-row center-hor"
        v-if="this.isActivityClicked"
        @click="markActive('output-data')"
      >
        <div
          class="connecting-to-center-line-hor"
          v-bind:class="{outputDataMarkLine: isOutputDataSelected}"
        ></div>
...
.selected {
  ...
  box-shadow: 0 5px 10px rgba(154, 160, 185, 0.05),
    0 15px 40px rgba(245, 111, 111, 0.2);
}
...
.inputDataSelected {
  ...
  box-shadow: 0 5px 10px rgba(154, 160, 185, 0.05),
    0 15px 40px rgba(255, 22, 22, 0.2);
}
...
.inputItSelected {
  ...
  transition: background-color 0.5s;
  box-shadow: 0 5px 10px rgba(154, 160, 185, 0.05),
    0 15px 40px rgba(253, 188, 245, 0.2);
}
...
.outputDataSelected {
  ...
  transition: background-color 0.5s;
  box-shadow: 0 5px 10px rgba(154, 160, 185, 0.05),
    0 15px 40px rgba(53, 255, 27, 0.2);
}

.marked {
  background-color: #5db4ff !important;
  box-shadow: 0 5px 10px rgba(154, 160, 185, 0.05),
    0 15px 40px rgba(33, 85, 255, 0.2);
  transition: background-color 0.5s;
}
